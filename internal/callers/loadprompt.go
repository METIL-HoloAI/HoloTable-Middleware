...
// InitializeChat loads chat settings and sends the initPrompt to ChatGPT
func InitializeChat() {
	// Load chat settings
	chatSettings, err := configloader.GetChat()
	if err != nil {
		fmt.Println("Error loading chat settings:", err)
		return
	}
	...
	for _, file := range yamlFiles {
		...
		if err != nil {
			fmt.Println("Error reading YAML file:", file, err)
			return
		}
...
Remember, from now on you will only send me json files.%s`, yamlContents)
	...
	fmt.Println("Chat API Response:", string(body))
}
...
func LoadPrompt(prompt string) (map[string]interface{}, error) {
	...
	// Convert payload to JSON // might not need this
	payloadBytes, err := json.Marshal(payload)
	if err != nil {
